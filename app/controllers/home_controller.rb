 class HomeController < ApplicationController
  ...
  def contracts_meter 
    ...
    if current_user.admin?
      ...
      @resume = Cna.find_by_sql(['select count(1), sum(amount), u.name from cnas c, taxpayers t, cities ct, users u where c.taxpayer_id = t.id and t.user_id = u.id and c.status = 0 and t.city_id = ct.id and ct.fl_charge = ? AND t.client_id = ? group by u.name order by u.name', true, session[:client_id]])
      ...
      @resume = Cna.find_by_sql(['select count(1), sum(amount), u.name from cnas c, taxpayers t, cities ct, users u where c.taxpayer_id = t.id and t.user_id = ? and t.user_id = u.id and c.status = 0 and t.city_id = ct.id and ct.fl_charge = ? AND t.client_id = ? group by u.name', current_user.id, true, session[:client_id] ])
    end
