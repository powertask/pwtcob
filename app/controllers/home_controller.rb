 class HomeController < ApplicationController
  ...
  def contracts_meter 
    ...
    if current_user.admin?
      @count_contracts_deal_day   = Contract.list_not_cancel(current_user.unit_id, session[:client_id]).active.where('contract_date between ? AND ?', Date.current.beginning_of_day, Date.current.end_of_day).count
      @count_contracts_deal_month = Contract.list_not_cancel(current_user.unit_id, session[:client_id]).active.where('contract_date between ? AND ?', dt_ini, dt_end ).count
      ...
      @count_contracts_deal_day   = Contract.list_not_cancel(current_user.unit_id, session[:client_id] ).active.where('user_id = ? AND contract_date between ? AND ?', current_user.id, Date.current.beginning_of_day, Date.current.end_of_day).count
      @count_contracts_deal_month = Contract.list_not_cancel(current_user.unit_id, session[:client_id] ).active.where('user_id = ? and contract_date between ? AND ?', current_user.id, dt_ini, dt_end ).count
