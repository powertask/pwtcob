 class HomeController < ApplicationController
  ...
  def contracts_meter 
    ...
    if current_user.admin?
      ...
      @resume = Cna.find_by_sql('select count(1), sum(amount), u.name from cnas c, taxpayers t, users u where c.taxpayer_id = t.id and t.user_id = u.id and status = 0 group by u.name')
      ...
      @resume = Cna.find_by_sql(['select count(1), sum(amount), u.name from cnas c, taxpayers t, users u where c.taxpayer_id = t.id and t.user_id = ? and t.user_id = u.id and status = 0 group by u.name', current_user.id ])
    end
